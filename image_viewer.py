...
class HighPerformanceImageView(QGraphicsView):
    ...
    def draw_grid(self, painter):
        ...
        try:
            ...
            src_points = np.array([[pt.x(), pt.y()] for pt in self.nodes], dtype=np.float32)
            ...
            if self.grid_h_reps > 1:
                for i in range(1, self.grid_h_reps):
                    ...
                    line_points = []
                    for y in range(0, ref_height + 1, max(1, ref_height // 50)):  # 50 segments per line
                        rect_point = np.array([[[x, y]]], dtype=np.float32)
                        orig_point = cv2.perspectiveTransform(rect_point, transform_matrix)[0][0]
                        view_point = self.mapFromScene(QPointF(orig_point[0], orig_point[1]))
            ...
            if self.grid_v_reps > 1:
                for i in range(1, self.grid_v_reps):
                    ...
                    line_points = []
                    for x in range(0, ref_width + 1, max(1, ref_width // 50)):  # 50 segments per line
                        rect_point = np.array([[[x, y]]], dtype=np.float32)
                        orig_point = cv2.perspectiveTransform(rect_point, transform_matrix)[0][0]
                        view_point = self.mapFromScene(QPointF(orig_point[0], orig_point[1]))
                        line_points.append(view_point)
