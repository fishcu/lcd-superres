...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.refine_button.setToolTip("Automatically refine corner points by minimizing the brightness of the grid lines. Assumes TL, TR, BR, BL order.")
        ...
        self.blur_sigma_spinbox.setToolTip("Sigma for Gaussian blur before darkness optimization.")
    ...
    def update_blur_preview(self):
        ...
        source_patch[dst_y_start_patch:dst_y_end_patch, dst_x_start_patch:dst_x_end_patch] = \
            image_gray[src_y_start_img:src_y_end_img, src_x_start_img:src_x_end_img]
        ...
        final_crop = blurred_patch[margin:margin+preview_size, margin:margin+preview_size]
        
        # Convert to QImage/QPixmap for display
        norm_crop = (final_crop * 255).clip(0, 255).astype(np.uint8)
        q_image = QImage(norm_crop.data, norm_crop.shape[1], norm_crop.shape[0], norm_crop.shape[1], QImage.Format.Format_Grayscale8)
    ...
    def start_corner_refinement(self):
        ...
        if pil_image is None or len(self.image_viewer.nodes) != 4:
            self.refinement_status_label.setText("Error: Requires an image and 4 corner points.")
            return
        ...
        self.refinement_thread.progress.connect(self.refinement_status_label.setText)
