...
class SuperResolutionWindow(QMainWindow):
    def __init__(self, main_window):
        ...
        self.image_label = QLabel(
            "Click 'Compute' to generate super-resolution image.")
    ...
    def start_computation(self):
        ...
        if pil_image is None or len(quad_points) != 4:
            self.status_label.setText(
                "Error: Requires an image and 4 points in the main window.")
            self.compute_button.setEnabled(True)
            self.progress_bar.setVisible(False)
            return

        # Run computation in a separate thread
        self.thread = SuperResolutionThread(
            pil_image, quad_points, h_reps, v_reps, output_size)
